/**
 * GameUtils - Shared Game Utility Functions
 * Used by multiple overlays to avoid code duplication
 * Static class to avoid polluting global namespace
 */
...
class GameUtils {
     ...
     */
    static calculateSecondsUntilStart(gameDate) {
     ...
     */
    static formatLiveGameStatus(statusText) {
...
// Expose globally for backwards compatibility (will be removed in future)
window.formatCountdown = GameUtils.formatCountdown.bind(GameUtils);
window.detectGameState = GameUtils.detectGameState.bind(GameUtils);
window.calculateSecondsUntilStart = GameUtils.calculateSecondsUntilStart.bind(GameUtils);
window.formatLiveGameStatus = GameUtils.formatLiveGameStatus.bind(GameUtils);
